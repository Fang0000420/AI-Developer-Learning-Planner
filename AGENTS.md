...
Day 12 已完成并在服务器验收通过：后端新增 `progress_logs` 表和进度提交/查询接口，`POST /api/progress` 可保存指定计划与指定日的完成任务、未完成任务、阻塞项和用户反馈，并同步每日任务状态；`GET /api/progress/{planId}?dayIndex={dayIndex}` 可查询历史提交记录。前端 `/plans/[planId]/today` 已增加进度提交区域，支持勾选完成/未完成任务、填写反馈和阻塞项，提交后刷新仍可看到最近一次记录。
...
- Day 12 的 `progress_logs.review_result_json` 目前只写入空对象，留给 Day 13 Progress Reviewer 回填或新增 reviewer 结果结构；当前页面只展示用户主动提交内容。
