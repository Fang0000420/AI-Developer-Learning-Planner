...
- 前端目标详情页的 `Generate Learning Plan` 按钮已接入真实生成流程，成功后跳转 `/plans/[planId]`；新增 `POST /api/plans/generate`、`GET /api/plans/[planId]` 同源 API Route；新增 `/plans/[planId]` 计划总览页，按 Day 展示主题、任务数、预计时长、任务类型、优先级和交付物。
