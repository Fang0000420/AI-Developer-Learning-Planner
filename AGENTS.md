...
Day 11 已完成并在服务器验收通过：后端新增每日任务查询与状态更新接口，支持按计划查询全部任务、按 `dayIndex` 查询今日/指定日任务，并通过 plan-scoped 接口更新任务状态；任务状态统一为 `PENDING`、`IN_PROGRESS`、`DONE`、`SKIPPED`。前端新增 `/plans/[planId]/today` 每日任务页面，可查看某一天任务、切换上一天/下一天并更新任务状态；`/tasks/today` 会跳转到最新可用计划；`/plans` 和 `/plans/[planId]` 均增加每日任务入口。
