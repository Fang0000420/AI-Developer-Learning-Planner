...
function backendUnavailableResponse(error: unknown) {
  const message =
    error instanceof Error
      ? error.message
      : "Backend project recommendation API is unavailable.";

  return NextResponse.json(
    {
      errors: { backend: message },
      message: "Backend project recommendation API is unavailable.",
      status: "BAD_GATEWAY",
...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(
      `${getBackendBaseUrl()}/api/goals/${goalId}/project-recommendation/recommend`,
      {
        cache: "no-store",
        method: "POST",
      },
