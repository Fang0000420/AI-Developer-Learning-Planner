...
function getParams(request: Request) {
  ...
  return {
    ...
    version: versionsIndex >= 0 ? parts.at(versionsIndex + 1) : undefined,
  };
}
...
function backendUnavailableResponse(error: unknown) {
  const message =
    error instanceof Error
      ? error.message
      : "Backend learning plan version API is unavailable.";

  return NextResponse.json(
    {
      errors: { backend: message },
      message: "Backend learning plan version API is unavailable.",
      status: "BAD_GATEWAY",
...
export async function POST(request: Request) {
  try {
    const { planId, version } = getParams(request);
    const response = await fetch(
      `${getBackendBaseUrl()}/api/plans/${planId}/versions/${version}/restore`,
      {
        cache: "no-store",
