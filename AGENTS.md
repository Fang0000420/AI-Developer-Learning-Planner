...
Day 10 已完成并在服务器验收通过：前端目标详情页可以从项目推荐卡片触发学习计划生成，后端调用 Agent 服务生成 14/21 天计划，保存 `Plan Generator` 的 `agent_runs` 记录，并将完整计划保存到 `learning_plans.plan_json`、每日任务保存到 `daily_tasks`。前端新增 `/plans/[planId]` 计划总览页和 `/plans` 计划列表页，可查看所有已生成计划并进入详情；`/goals` 与 `/plans` 列表每条记录均支持暂停/激活和删除。
