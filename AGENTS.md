...
- 新增 `.github/workflows/ci.yml`，在 PR 和 `main` push 时分别运行 backend、agent-service、frontend 三组检查；backend job 使用 Java 17，并启动 PostgreSQL 16 和 Redis 7 service。
- 前端新增 Vitest + Testing Library 测试基座，`frontend/package.json` 新增 `npm test`；测试覆盖 `/goals/new` 目标表单校验和后端 API 错误展示，以及 `/plans/[planId]/today` 今日任务列表渲染和任务 API 错误状态。
- Agent 服务新增 `tests/test_schema_contracts.py`，覆盖 profile、goal、skill-gap、project、plan、progress 和 adjust 相关 Pydantic schema 的有效输入、无效输出和字段归一化行为。
- 后端补充 health/request id 测试，验证响应会生成 `X-Request-Id`，并会回写调用方传入的 request id。
- README 新增“测试与 CI”说明，列出服务器上三端测试命令和 CI 触发范围；`.github/workflows/README.md` 从占位说明更新为当前 CI 说明。
- 本机验证：后端临时指定 `JAVA_HOME=C:\Program Files\Java\jdk-17` 后 `mvn test` 通过，`Tests run: 90, Failures: 0, Errors: 0, Skipped: 0`；Agent `.venv` 下 `pytest` 通过，`31 passed, 1 warning`，`ruff check .` 通过；前端 `npm test` 通过，`4 passed`，`npm run lint`、`npm run typecheck`、`npm run format:check`、`npm run build` 均通过。
...
- Day 19 后三端测试命令已固定：后端 `cd backend && mvn test`，Agent `cd agent-service && source .venv/bin/activate && pytest && ruff check .`，前端 `cd frontend && npm test && npm run lint && npm run typecheck && npm run format:check && npm run build`。GitHub Actions 会在 PR 和 `main` push 时运行同等检查。
